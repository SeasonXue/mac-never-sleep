...
3. Keep `PanelView::Main` / `Settings` / `Help`. Match `docs/screenshots` (centered coin, Start pill, More Settings | Quit). The grouped card lives on Settings (duration + switches). Glass belongs on the **panel chrome**; cards stay opaque. Anchor under the status item and hide on focus loss. Do not gray out End Standby or Sleep Display Now.
