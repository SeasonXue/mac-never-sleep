...
左键点一下菜单栏的太阳即可打开紧凑的**工具面板**（侧边栏 + 详情，不吸附在图标旁）。待命页点 **开始关屏待命** —— 状态图标翻成月亮，约 1.5 秒后屏幕关闭。关掉窗口或再点图标只是隐藏面板，不会结束待命；待命中 **结束待命** 始终可点。屏幕、合盖、保护、语言各自在侧边栏分区里。
...
菜单栏默认显示拟人化太阳图标。左键打开工具面板，再点 **开始关屏待命**。状态图标会翻成月亮，约 1.5 秒后屏幕关闭。点 **结束待命** 即可停止。关掉窗口（或再点图标）只是隐藏面板，不会结束待命。右键图标仍可打开原生备用菜单（含显示窗口、设置）。Finder 显示 **Never Sleep**。
